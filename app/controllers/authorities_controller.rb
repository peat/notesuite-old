class AuthoritiesController < ApplicationController
  ...
  # GET /authorities.xml
  def index
    @authorities = Authority.all.paginate :page => params[:page]
  ...
  def show
    ...
    @master = Master.new # for hidden form
  ...
  def create
    ...
    respond_to do |format|
      if @authority.save
        session[:last_authority] = @authority.id
        flash[:notice] = "Authority #{@authority.name} was successfully created."
  ...
  def update
    ...
    respond_to do |format|
      if @authority.update_attributes(params[:authority])
        ...
        format.html { redirect_to(@authority) }
        format.xml  { head :ok }
      else
