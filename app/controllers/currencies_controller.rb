class CurrenciesController < ApplicationController
  ...
  def show
    ...
    @master = Master.new( :currency => @currency ) # for hidden form
  ...
  def create
    ...
    respond_to do |format|
      if @currency.save
        session[:last_currency] = @currency.id
        session[:last_authority] = @currency.authority.try(:id)
        flash[:notice] = "Currency '#{@currency.name}' was successfully created."
